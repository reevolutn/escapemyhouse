...
sv = {
	...
	createScene : function () {
		...
		//create
		sv.scene = new BABYLON.Scene(u.engine);
		sv.scene.clearColor = new BABYLON.Color3(0, 0, 0);

		//CAMERAS
	    //mobile - NON-VR 
	    sv.cameras.mobile = new BABYLON.DeviceOrientationCamera("camera1", new BABYLON.Vector3(0, 0, 0), sv.scene);
	    sv.cameras.mobile.fov = 1;
	    
	    //mobile - VR 
	    sv.cameras.vr = new BABYLON.VRDeviceOrientationFreeCamera("camera2", new BABYLON.Vector3(0, 0, 0), sv.scene, true);
	    sv.cameras.vr.fov = 1;

	    //desktop
	    sv.cameras.desktop = new BABYLON.FreeCamera("camera_desktop", new BABYLON.Vector3(0, 0, 0), sv.scene);
	    sv.cameras.desktop.fov = 1.5;

	    if(u.camera == 'vr') {
	   		sv.scene.activeCamera = sv.cameras.vr;
	   		sv.activeCamera = 'vr';
	    } else if(u.camera == 'mobile') {
	   		sv.scene.activeCamera = sv.cameras.mobile;
	   		sv.activeCamera = 'mobile';
	    } else {
	   		sv.scene.activeCamera = sv.cameras.desktop;
	   		sv.activeCamera = 'desktop';
	    }

	    //attach control
	    sv.scene.activeCamera.attachControl(u.canvas, true);
	...
	populateGoogleImage : function(img1) {
		...
		//fix for iOS
		$('#video_content').html('');
		$('#video_content').html('<video id="video" muted playsinline loop preload="auto" src="/assets/video/stream.php?url=house_smoke.mp4" style="display:none"></video>');
		var videosmoke = document.getElementById('video');
		videosmoke.load();
		makeVideoPlayableInline(videosmoke);
		videosmoke.onloadeddata = function() {
		    ...
		    sv.objects.smokeVideoMat.diffuseTexture.video.play();
		};

		//fix for iOS
		$('#video_content2').html('');
		$('#video_content2').html('<video id="video2" muted playsinline preload="auto" loop src="/assets/video/stream.php?url=house_fire.mp4" style="display:none"></video>');
		var videofire = document.getElementById('video2');
		makeVideoPlayableInline(videofire);
		videofire.load();
		videofire.onloadeddata = function() {
		    ...
		    sv.objects.fireVideoMat.diffuseTexture.video.play();
		};
